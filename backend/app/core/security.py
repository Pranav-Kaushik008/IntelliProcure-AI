...
def get_supplier_for_user(current_user, db: Session):
    ...
    Matches by email, contact person, department/company name, or demo aliases.
    """
    ...
    # 2. Match by contact person name
    ...
    # 3. Match by department / company name
    if current_user.department:
        supplier = db.query(Supplier).filter(
            Supplier.company_name.ilike(f"%{current_user.department}%"),
            Supplier.is_deleted == False
        ).first()
        if supplier:
            return supplier

    # 4. Fallback for demo supplier logins
    if "supplier" in current_user.email.lower() or "cloudlogix" in current_user.email.lower():
        supplier = db.query(Supplier).filter(
            Supplier.company_name.ilike("%CloudLogix%"),
            Supplier.is_deleted == False
        ).first()
        if supplier:
            return supplier

    return db.query(Supplier).filter(Supplier.is_deleted == False).first()
