...
from app.core.security import (
    get_current_active_user, require_roles, require_internal_user,
    normalize_role, get_supplier_for_user
)
...
async def list_suppliers(
    ...
    """
    List suppliers.
    - Internal users: see all suppliers with full filtering.
    - Supplier role: sees only their own linked supplier record.
    """
    user_role = normalize_role(current_user.role)

    # Supplier-role: return only their own supplier profile
    if user_role == "supplier":
        own_supplier = get_supplier_for_user(current_user, db)
        if not own_supplier:
            return []
        return [own_supplier]

    # Internal staff: full listing with filters
