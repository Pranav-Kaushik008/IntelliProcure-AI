...
def recommend_supplier_internal(rfq_id: UUID, db: Session) -> dict:
    """
    Module 8: Explainable AI Multi-Criteria Supplier Recommendation Engine.
    Evaluates bids for an RFQ across Price, Quality, Delivery, Reliability, and Risk factors.
    Returns structured rankings array, top recommendation, confidence, and reasons.
    """
    rfq = db.query(RFQ).filter(RFQ.id == rfq_id).first()
    if not rfq:
        return {"rfq_id": str(rfq_id), "rankings": [], "top_recommendation": None}

    quotes = db.query(Quotation).options(joinedload(Quotation.supplier)).filter(
        Quotation.rfq_id == rfq_id,
        Quotation.is_deleted == False
    ).all()

    # Fallback to all suppliers if no quotes exist yet
    evaluated_suppliers = []
    if quotes:
        ...
        for q in quotes:
            ...
            # Price Score: 0 to 100 (relative to best price)
            price_ratio = min_price / max(q.total_amount, 1.0)
            price_score = round(min(max(price_ratio * 100.0, 10.0), 100.0), 1)

            # Delivery Score: 0 to 100
            deliv_ratio = min_delivery / max(q.delivery_days or 14, 1.0)
            delivery_score = round(min(max(deliv_ratio * 100.0, 20.0), 100.0), 1)

            # Quality Score (from supplier rating 1-5 -> mapped to 0-100)
            quality_score = round(min(max((sup.quality_score or 4.0) * 20.0, 0.0), 100.0), 1)

            # Reliability & Rating
            rating_score = round(min(max((sup.overall_rating or 4.0) * 20.0, 0.0), 100.0), 1)

            # Risk Score (Inverse of risk: lower risk score means higher safety score)
            safety_score = round(min(max(100.0 - (sup.risk_score or 20.0), 10.0), 100.0), 1)

            # Weighted Composite Score
            # Price 35%, Quality 25%, Delivery 15%, Reliability 15%, Safety 10%
            total_score = round(
                (price_score * 0.35) +
                (quality_score * 0.25) +
                (delivery_score * 0.15) +
            ...
            reasons = []
            if price_ratio >= 0.95:
                reasons.append(f"Highly competitive pricing (${q.total_amount:,.2f}) within 5% of benchmark.")
            if quality_score >= 80.0:
                reasons.append(f"Proven quality track record ({sup.quality_score:.1f}/5.0 score).")
            if (sup.risk_score or 20.0) <= 25.0:
                reasons.append("Low enterprise supplier risk profile & validated compliance.")
            if delivery_score >= 85.0:
                reasons.append(f"Fast delivery turnaround ({q.delivery_days} days).")
            if not reasons:
                reasons.append("Balanced price-to-performance ratio across all evaluation criteria.")

            evaluated_suppliers.append({
                "supplier": {
                    "id": str(sup.id),
                    "company_name": sup.company_name,
                    "supplier_code": sup.supplier_code or "SUP-000",
                    "category": sup.category or "General",
                    "overall_rating": sup.overall_rating or 4.0,
                    "risk_level": sup.risk_level or "low",
                    "risk_score": sup.risk_score or 20.0
                },
                "quote": {
                    "id": str(q.id),
                ...
                "reasons": reasons,
                "explanation": f"{sup.company_name} achieved an AI score of {total_score}/100 with optimal pricing (${q.total_amount:,.2f}) and {(100.0 - (sup.risk_score or 20.0)):.0f}% safety score."
            })
    else:
        # Evaluate baseline suppliers
        all_sups = db.query(Supplier).filter(Supplier.is_deleted == False).limit(5).all()
        for sup in all_sups:
            rating_score = round(min(max((sup.overall_rating or 4.0) * 20.0, 0.0), 100.0), 1)
            quality_score = round(min(max((sup.quality_score or 4.0) * 20.0, 0.0), 100.0), 1)
            safety_score = round(min(max(100.0 - (sup.risk_score or 20.0), 10.0), 100.0), 1)
            total_score = round((quality_score * 0.4) + (rating_score * 0.3) + (safety_score * 0.3), 1)
            ...
            evaluated_suppliers.append({
                ...
                "reasons": [
                    f"Strong historical supplier rating ({sup.overall_rating:.1f}/5.0).",
                    f"Established quality standard ({sup.quality_score:.1f}/5.0).",
                    "Awaiting formal quotation submission for this RFQ."
                ],
                "explanation": f"Recommended based on historical performance scorecard rating ({sup.overall_rating:.1f}/5.0)."
            })

    # Sort descending by score and assign ranks
    evaluated_suppliers.sort(key=lambda x: x["score"], reverse=True)
    for idx, item in enumerate(evaluated_suppliers):
        item["rank"] = idx + 1
    ...
    return {
        ...
        "title": rfq.title,
        "evaluated_count": len(evaluated_suppliers),
        "rankings": evaluated_suppliers,
        "top_recommendation": evaluated_suppliers[0] if evaluated_suppliers else None
    }
