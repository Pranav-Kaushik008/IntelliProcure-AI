...
export default function PurchaseRequestsPage() {
  ...
  // Form state – numeric fields stored as strings to avoid leading-zero artefacts
  ...
  const handleSavePR = async (asDraft = false) => {
    ...
    const estAmount = Number(formData.estimated_amount);
    if (!formData.estimated_amount || isNaN(estAmount) || estAmount <= 0) {
  ...
  const handleOpenEdit = (pr) => {
    ...
    setFormData({
      ...
      quantity: pr.items?.[0]?.quantity != null ? String(pr.items[0].quantity) : "",
      estimated_amount: pr.estimated_amount != null ? String(pr.estimated_amount) : "",
  ...
  if (isSupplier) {
    return (
      <div className="card" style={{ padding: 40, textAlign: "center", maxWidth: 560, margin: "60px auto" }}>
        ...
        <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>Internal Enterprise Requisitions</h3>
        <p style={{ color: "var(--text-muted)", fontSize: 14, lineHeight: 1.6, marginBottom: 20 }}>
          Purchase Requests are internal employee requisitions. As a registered supplier partner, please manage your assigned Purchase Orders, RFQ bids, and Invoices.
        </p>
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          <a href="/purchase-orders" className="btn btn-primary">View Purchase Orders</a>
          <a href="/rfqs" className="btn btn-secondary">Browse RFQs</a>
        </div>
      </div>
    );
  }
