...
export default function PurchaseOrdersPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: "",
    items: [
      { item_name: "", quantity_ordered: "", unit_price: "", discount_rate: "", tax_rate: "" }
    ]
  });
  ...
  const handleCreatePO = async (e, asDraft = true) => {
    ...
    // Validate line items — prevent zero-value POs
            ...
            <motion.div className="card" style={{ width: 640, padding: 24, maxHeight: "90vh", overflowY: "auto" }}>
              ...
              <form style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div>
                  ...
                  {/* Live Estimated Total Preview */}
                  {(() => {
                    const est = formData.items.reduce((acc, it) => {
                      ...
                      return acc + (sub - discAmt + taxAmt);
                    }, 0);
                    return (
                      <div style={{
                        display: "flex", justifyContent: "flex-end", alignItems: "center",
                        marginTop: 6, padding: "8px 12px", borderRadius: 8,
                        background: est > 0 ? "rgba(34,197,94,0.08)" : "rgba(239,68,68,0.08)",
                        border: `1px solid ${est > 0 ? "rgba(34,197,94,0.25)" : "rgba(239,68,68,0.3)"}`,
                      }}>
                        <span style={{ fontSize: 12, color: "#94a3b8", marginRight: 10 }}>
                          Estimated Total (incl. tax &amp; discount):
                        </span>
                        <span style={{
                          fontWeight: 700, fontSize: 15,
                          color: est > 0 ? "#22c55e" : "#ef4444"
                        }}>
                          ${est.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                        </span>
                      </div>
                    );
                  })()}
