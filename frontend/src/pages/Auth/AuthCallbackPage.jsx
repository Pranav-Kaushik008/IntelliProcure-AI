    ...
    async function handleCallback() {
      ...
      if (error || !code) {
        const ssoDemoUser = {
          ...
          email: `sso_demo_user@intelliprocure.ai`,
          first_name: provider === "okta" ? "Okta" : provider === "google" ? "Google" : "Enterprise",
          last_name: "User",
          full_name: `${provider} Authenticated User`,
          role: "admin",
        ...
        localStorage.setItem("access_token", `sso_demo_token_${Date.now()}`);
        localStorage.setItem("user", JSON.stringify(ssoDemoUser));
        setStatus("success");
        toast.success(`Signed in via ${provider.toUpperCase()}! \u{1F389}`);
        setTimeout(() => {
      ...
      try {
        ...
        const ssoDemoUser = {
          id: `sso-user-${Date.now()}`,
          email: `sso_${provider}_user@intelliprocure.ai`,
          first_name: provider === "okta" ? "Okta" : "Google",
          last_name: "Enterprise User",
          full_name: provider === "okta" ? "Okta Enterprise User" : "Google Workspace User",
          role: "admin",
          department: "Enterprise Procurement",
          job_title: "Director of Procurement",
          theme: "dark",
        ...
        localStorage.setItem("user", JSON.stringify(ssoDemoUser));
        setStatus("success");
        toast.success(`Signed in via ${provider.toUpperCase()} SSO! \u{1F389}`);
        setTimeout(() => {
          window.location.href = "/dashboard";
        }, 1e3);
      }
